...
public class FXMLController {
	...
	void doCalcolaPercorso(ActionEvent event) {
		try {
			String array[] = boxArco.getValue().split(" ");
			String source = array[0];
			String target = array[1];
			List<String> percorso = model.calcolaPercorso(source, target);
	...
	void doCreaGrafo(ActionEvent event) {
		try {
			...
			Graph<String, DefaultWeightedEdge> grafo = model.creaGrafo(boxMese.getValue(), boxCategoria.getValue());
			txtResult.appendText("Grafo creato con " + grafo.vertexSet().size() + " vertici e " + grafo.edgeSet().size()
					+ " archi.");

			for (String s : model.archiSopraLaMedia(grafo))
				boxArco.getItems().add(s);
	...
	public void setModel(Model model) {
		this.model = model;
		boxCategoria.getItems().addAll(model.getCategorieReati());
